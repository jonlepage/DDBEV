...
const OsMenue = () => {
	...
	const File = store({
		...
		list: [
			...
			{ label: '-Load Projet', iconPath: 'new_proj.png' },
			null,
			{ label: '-save', iconPath: 'new_proj.png' },
			{ label: '-Save as', iconPath: 'new_proj.png' },
			...
			{File.list.map((data, i) => {
				if (data) {
					return (
						<Menu.Item key={i}>
							<Text type='secondary'>
								<img className={} height={16} src={data.iconPath} alt='Logo' />
								{data.label}
							</Text>
						</Menu.Item>
					);
