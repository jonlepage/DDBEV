...
function getModaleType(type) {
	switch (type) {
		case Store_treeSheets.TYPE.FOLDER:
			break;
		case Store_treeSheets.TYPE.FOLDER:
			break;
		default:
...
export const TreeTool = ({ type, _nodeHover }) => {
		...
		<>
			...
			>
				<div className={'TopModuleR_tool'}>
					...
					<FileAddFilled onClick={onClick_folder} />
				</div>
			</div>
			...
			{treeRoot.map((data, i) => (
				<div key={i}>
					<Collapse bordered={false} defaultActiveKey={['1']}>
						<TreeTool type={data[0].title} _nodeHover={_nodeHover}></TreeTool>
						<Panel header='' key='1'>
							<Tree
