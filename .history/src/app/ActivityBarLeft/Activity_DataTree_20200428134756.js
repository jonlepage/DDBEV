...
const Activity_DataTree = () => {
	...
	const [_nodeHover, _set_nodeHover] = useState({ node: null, offset: 0 });

	const onSelect = (selectedKeys, info) => {
		_set_toolY(info.nativeEvent.target.offsetTop);
	};
	const onMouseEnter = (info) => {
		_set_nodeHover({ node: info.node, offset: info.event.target.offsetTop });
		_set_toolY(info.event.target.offsetTop);
		_set_nodeHover(info.node.pos);
	};
