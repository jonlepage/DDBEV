...
const Activity_Modules = () => {
	...
	/** Store module layout */
	const Store_Module_inputString = store({
		...
		/** tous les layout creer */
		data: [],
		createView(id){return <Inputs_string dataId={id} />}
		/** create un new data */
		create(){}
		onDragStart() {},
	});
	/** Store Componment */
	const Store_Modules = store({ data: [Store_Module_layout] });

	return (
		<div className={'Activity_Modules'}>
			<Space direction='vertical'>

				{Store_Modules.data.map(data=>{<div
									id={'Inputs_string'}
									draggable={true}
