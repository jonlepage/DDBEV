...
export const Store_Tool_options = store({
	...
	onClick(id) {
		...
		if (index > -1) {
			Store_Tool_options.data.value.splice(index, 1);
		} else {
			Store_Tool_options.data.value = [...this.data.value, id];
		}
	},
	isActive(id) {
...
export const Store_ToolsEditor = store({
	...
	data: [Store_Tool_mode, Store_Tool_options, Store_Tool_Zoom], //, Store_Tool_options, Store_Tool_Zoom
});
