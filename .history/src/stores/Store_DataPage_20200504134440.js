...
export const Store_layouts = store({
	...
	MAKEDEMODEBUG(root = true, SuperRoot = 'CLASSES-folderA') {
		...
		for (let i = 0, l = 10; i < l; i++) {
			...
			fakeData.childrens = [`_fakeSubData_${i}`];
			Store_layouts.data[fakeId] = fakeData;
		}
		for (let i = 0, l = 10; i < l; i++) {
			const fakeId = `_fakeSubData_${i}`;
			const fakeData = { ...this.default };
			fakeData.isRoot = false;
			fakeData.id = fakeId;
			fakeData.level = 1;
			fakeData.key = `${SuperRoot}-${fakeId}-sub`;
			fakeData.childrens = [`_fakeSub2Data_${i}`];
			Store_layouts.data[fakeId] = fakeData;
		}
		for (let i = 0, l = 10; i < l; i++) {
			const fakeId = `_fakeSub2Data_${i}`;
			const fakeData = { ...this.default };
			fakeData.isRoot = false;
			fakeData.id = fakeId;
			fakeData.level = 2;
...
export const Store_layoutSettings = store({
	...
	getById(id) {
		return Store_layoutSettings.default;
	},
});
