	...
	children: [
		{
			...
			tooltip: 'ZoomIn',
			onClick() {
				const value = Store_Tool_Zoom.value;
				const max = Store_Tool_Zoom.max;
				Store_Tool_Zoom.value = Math.min(max, value + 0.1);
			},
		},
		{
			id: 'zoomOut',
			...
			onClick() {
				const value = Store_Tool_Zoom.value;
				const min = Store_Tool_Zoom.min;
				Store_Tool_Zoom.value = Math.max(value, value - 0.1);
			},
		},
	],
});
