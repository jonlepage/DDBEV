...
export const Store_treeSheets = store({
	/** selection dun node dans le tree */
	node_selected: '',
	/** survole dun node dans le tree */
	node_hover: '',
	...
	getTreeAntdFormat() {
		function mapperToAntd(data, key) {
			...
			const icon = Store_treeSheets.getIcon(data.dataType);
			return {
				...data,
				// antD tree structure
				title: data.id,
	...
	createData(data) {
		const Id = data.Id;
		Store_treeSheets.data[Id] = {
			rootId: 'CLASSES',
			id: Id,
			dataType: '.CLASSES||.EXTENT',
			contentId: 'fakeData_1',
			/** Si a des childrends, ce sont des .ext seulment visuel */
			children: [],
		};
		this.ROOT.CLASSES.children.push(Id);
	},
});
